...
export function ProductsPage() {
  ...
  const { products, loading, load, remove } = useProducts()
  const [search, setSearch] = useState('')
  const [editTarget, setEditTarget] = useState<Product | null>(null)
  const [showForm, setShowForm] = useState(false)

  useEffect(() => { load(isAdmin) }, [load, isAdmin])
  ...
  return (
    <div className="px-4 py-5">
      ...
      {!loading && filtered.length === 0 && (
        <EmptyState icon={<Package size={40} />} title="No products yet" description="Add your first product to get started" />
      )}
      ...
      <div className="flex flex-col gap-3">
        {filtered.map(product => (
          <Card key={product.id} className="flex items-center gap-3">
            {product.imageUrl
              ? <img src={product.imageUrl} alt={product.name} className="w-12 h-12 rounded-xl object-cover bg-surface-muted" />
              : <div className="w-12 h-12 rounded-xl bg-surface-muted flex items-center justify-center text-xl">🥬</div>
            }
      ...
      {showForm && (
        <ProductFormModal
          ...
          onSaved={() => { setShowForm(false); load(isAdmin) }}
        />
      )}
    </div>
  )
