...
export function BillDetailModal({ bill, onClose, onUpdated }: Props) {
  ...
  const { payments, loading, loadByBill, register } = usePayments()
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center" onClick={onClose}>
      <div
        className="bg-white w-full max-w-md rounded-t-2xl max-h-[92vh] overflow-y-auto"
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-surface-border">
          ...
          <div className="flex items-center gap-2">
            <button onClick={downloadPDF} className="p-2 rounded-lg bg-surface-muted text-ink-muted">
              <Download size={18} />
            </button>
              ...
              {payments?.map(p => (
                <div key={p.id} className="flex items-start gap-2 text-sm bg-surface-muted rounded-xl p-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-ink-muted">{p.mode.toUpperCase()} · {formatDate(p.paymentDate)} · by {p.registeredByUserName}</p>
                    {p.receiptImageUrl && (
                      <a href={p.receiptImageUrl} target="_blank" rel="noreferrer" className="text-xs text-primary-600 underline">
                        View receipt
                      </a>
                    )}
                  </div>
          ...
          {showForm && (
            <div className="border border-surface-border rounded-xl p-4 flex flex-col gap-3">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleRegister} loading={saving || uploading} className="flex-1">
                  Submit
                </Button>
              </div>
            </div>
